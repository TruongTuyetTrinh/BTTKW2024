...
def generate_nv5_data(n):
    nv5_data = []
    roles={
            'Quản lý': 10,
        'Nhân viên bán hàng': 50,
        'Nhân viên thu ngân': 20,
        'Nhân viên giao hàng': 20
        }
    index=0
    for chuc_vu, count in roles.items():
        for _ in range(count):
            if index>=n:break
            ma_nv = generate_ma_nv(index)
            ma_dn = generate_ma_dn(index)  # MaDN corresponding to MaNV
            ...
            if chuc_vu == 'Nhân viên giao hàng':
                    ...
                    bang_cap=random.choice([ 'Cao đẳng', 'Đại học'])
            else:
            ...
            gioi_tinh = 'Nam' if first_name in vietnamese_male_names else 'Nữ'
            ...
            nv5_data.append(insert_statement)
            index+=1
    return nv5_data

# Tạo 1000 dòng dữ liệu
generated_data = generate_nv5_data(100)

# Ghi dữ liệu SQL vào tệp nv_data.sql nếu chưa có
with open('D:/TKW/nv5_data.sql', 'w', encoding='utf-8') as file:
...
with open('D:/TKW/nv5_data.sql', 'r', encoding='utf-8') as file:
